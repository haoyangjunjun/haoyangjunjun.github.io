...
subtitle:   "上学期(2024)课余时间整理的笔记，也用于练习markdown语法 最近更新：2025-10-13"
...
```bash
# 1. 查看单个文件内容（全部输出）
cat /etc/passwd  # 直接打印/etc/passwd的所有行

# 2. 拼接多个文件内容（合并到新文件）
cat file1.txt file2.txt > merged.txt  # 将file1和file2的内容合并到merged.txt

# 3. 创建新文件（配合EOF输入多行内容）
cat > note.txt << EOF
这是第一行
这是第二行
EOF  # 输入结束，内容被写入note.txt
```

### `grep`：核心是“按条件筛选文本”
- **功能**：从文件或标准输入（如管道传递的内容）中，**搜索包含“指定模式（字符串/正则表达式）”的行**，并输出匹配的行。  
- 特点：不直接查看完整内容，而是“按需筛选”，只保留符合条件的行，常用于日志分析、内容检索。  

#### 典型用法：
```bash
# 1. 从文件中搜索包含"root"的行
grep "root" /etc/passwd  # 只输出/etc/passwd中含"root"的行

# 2. 从管道内容中筛选（结合其他命令）
cat /var/log/syslog | grep "error"  # 先通过cat读取日志，再筛选含"error"的行

# 3. 正则匹配（如搜索以"ssh"开头的行）
grep "^ssh" /etc/services  # ^表示行首，只匹配以ssh开头的行

# 4. 反向筛选（排除含"#"注释的行）
grep -v "^#" /etc/nginx/nginx.conf  # -v表示"不匹配"，输出非注释行
```

| 命令   | 核心功能                                  | 特点                                  | 典型场景                     |
|--------|-------------------------------------------|---------------------------------------|------------------------------|
| `cat`  | 读取/拼接文件内容，全量输出                | 简单直接，适合小文件                  | 快速查看短文件、合并文件     |
| `grep` | 按模式筛选文本，只输出匹配行              | 支持正则，擅长精准检索                | 日志分析、内容过滤           |
| `less` | 分页查看大文件（支持上下翻页、搜索）      | 不一次性加载全部内容，适合大文件      | 查看几GB的日志文件           |
| `more` | 分页查看文件（只能向下翻页）              | 功能比less简单                        | 简单分页查看                 |
| `head` | 输出文件的前N行（默认前10行）             | 快速获取文件开头内容                  | 查看日志的最新几条记录       |
| `tail` | 输出文件的后N行（默认后10行）             | 支持实时跟踪文件更新（`-f`参数）      | 实时监控日志新增内容         |
...
`umask` 是 Linux/Unix 系统中用于**设置“新建文件/目录默认权限”** 的重要命令，它通过“权限掩码”的方式，限制新创建文件/目录的初始权限，是系统安全的基础配置之一。


### umask 
是“权限减法器”
- 系统对文件和目录有**默认最大权限**：
  - 新建文件的最大默认权限：`666`（即 `-rw-rw-rw-`，文件默认不能执行，所以没有执行权限 `x`）。
  - 新建目录的最大默认权限：`777`（即 `drwxrwxrwx`，目录需要执行权限才能进入）。
- `umask` 的作用：**从最大权限中“减去”掩码值，得到实际默认权限**。  
  公式：  
  ```
  实际权限 = 最大默认权限 - umask值
  ```
...
#### umask 值的表示方式
`umask` 值通常用**3位八进制数字**表示（如 `022`、`007`），每一位对应文件权限的 **“所有者（u）”“所属组（g）”“其他用户（o）”** 三类角色，每一位的取值范围是 `0~7`（对应权限 `r=4`、`w=2`、`x=1` 的组合）。

例如 `umask 022`：
- 第一位 `0`：对所有者（u）的权限无限制（减 0）；
- 第二位 `2`：对所属组（g）减去写权限（`w=2`）；
- 第三位 `2`：对其他用户（o）减去写权限（`w=2`）。
...
以 `umask 022` 为例：  
```
文件最大权限：666（二进制 110 110 110）
umask值：     022（二进制 000 010 010）
实际权限 = 666 - 022 = 644（二进制 110 100 100）
即：-rw-r--r--（所有者可读可写，组和其他用户只读）
```

##### 2. 对目录的计算（最大权限 `777`）
同样 `umask 022`：  
```
目录最大权限：777（二进制 111 111 111）
umask值：     022（二进制 000 010 010）
实际权限 = 777 - 022 = 755（二进制 111 100 100）
即：drwxr-xr-x（所有者可读可写可进，组和其他用户可读可进）
```
...
##### 1. 查看当前 umask 值
```bash
umask  # 直接执行，输出当前掩码值（如 0022）
```
- 输出的 `0022` 中，第一位 `0` 是“特殊权限位”（通常忽略，默认0），后三位 `022` 是实际作用的掩码。

##### 2. 临时修改 umask（当前Shell有效）
```bash
umask 027  # 将当前Shell的umask改为027，新建文件/目录会按此计算权限
```
- 验证：  
  ```bash
  umask 027
  touch testfile  # 新建文件，权限应为 666-027=640（-rw-r-----）
  mkdir testdir   # 新建目录，权限应为 777-027=750（drwxr-x---）
  ```

##### 3. 永久修改 umask（全局/用户级）
- **全局配置**（对所有用户生效）：  
  修改 `/etc/profile` 或 `/etc/bashrc`，添加一行 `umask 022`，保存后重启Shell生效。
- **用户级配置**（仅对当前用户生效）：  
  修改用户主目录的 `.bashrc` 或 `.bash_profile`，添加 `umask 022`。


#### 常见 umask 取值及适用场景
| umask值 | 文件默认权限 | 目录默认权限 | 适用场景 |
|---------|--------------|--------------|----------|
| 000     | 666（-rw-rw-rw-） | 777（drwxrwxrwx） | 完全开放（危险，不建议） |
| 002     | 664（-rw-rw-r--） | 775（drwxrwxr-x） | 多用户共享目录（组内可写） |
| 022     | 644（-rw-r--r--） | 755（drwxr-xr-x） | 系统默认（安全，普通用户） |
| 027     | 640（-rw-r-----） | 750（drwxr-x---） | 敏感数据（仅所有者和组可见） |
| 077     | 600（-rw-------） | 700（drwx------） | 私有目录（仅所有者可见） |


#### 为什么需要 umask？
- **安全防护**：默认限制“其他用户”的写权限（如 `022` 确保文件不会被任意修改），避免新建文件/目录权限过松导致的信息泄露。
- **统一规范**：通过全局配置，保证系统中所有新建文件/目录的权限符合安全策略，减少人为配置失误。


#### 总结
`umask` 是通过“减法”从最大权限中限制默认权限的工具，核心公式为 `实际权限 = 最大权限 - umask值`。合理设置 `umask`（如系统默认的 `022`）是保障文件系统安全的基础，需根据数据敏感程度选择合适的掩码值。
...
## awk
### **awk核心功能**
- 按行处理文本，支持通过分隔符（如空格、冒号、逗号等）分割行内容为“字段”。
- 基于条件（模式）筛选行，并对符合条件的行执行指定操作（如打印、计算、替换等）。
- 支持变量、循环、函数等编程特性，可实现复杂的文本处理逻辑。

### **awk基本语法**
```bash
awk [选项] '模式 {动作}' 文件名
```
- **模式（Pattern）**：用于筛选行（如条件判断、正则匹配），只有满足模式的行才会执行后续“动作”。
- **动作（Action）**：对符合模式的行执行的操作（如 `print` 打印、计算等），需用 `{}` 包裹。
- **选项**：常用 `-F` 指定字段分隔符（默认以空格/制表符分隔），`-v` 定义变量等。

在 Linux/Unix 及 Shell 环境中，命令的 “成功 / 失败” 是通过 “退出状态码”（Exit Status Code）来定义的，这是一套通用的约定，grep 也遵循这套规则 ——  返回 0 是 命令执行成功 的标准标识。

`umask` 是 Linux/Unix 系统中用于**设置“新建文件/目录默认权限”** 的重要命令，它通过“权限掩码”的方式，限制新创建文件/目录的初始权限，是系统安全的基础配置之一。

## umask
### 一、核心概念：umask 是“权限减法器”
- 系统对文件和目录有**默认最大权限**：
  - 新建文件的最大默认权限：`666`（即 `-rw-rw-rw-`，文件默认不能执行，所以没有执行权限 `x`）。
  - 新建目录的最大默认权限：`777`（即 `drwxrwxrwx`，目录需要执行权限才能进入）。
- `umask` 的作用：**从最大权限中“减去”掩码值，得到实际默认权限**。  
...
## 软链接
在 Linux/Unix 系统中，**软连接（Symbolic Link，也叫符号链接）** 是一种特殊的文件，它类似于 Windows 系统中的“快捷方式”，用于指向另一个文件或目录。软连接本身是一个独立的文件，它存储的是目标文件的路径（可以是相对路径或绝对路径），而非目标文件的数据。


### 软连接的特点：
1. 软连接可以跨文件系统（如从 `/` 分区指向 `/home` 分区的文件）。
2. 软连接可以指向目录。
3. 当目标文件被删除或移动后，软连接会失效（变成“断链”，访问时会提示“文件不存在”）。
4. 软连接文件的权限通常显示为 `lrwxrwxrwx`（权限对其实际访问目标文件的权限无影响，目标文件的权限才起作用）。


### 创建软连接的命令：`ln -s`
基本语法：
```bash
ln -s 目标文件或目录的路径 软连接的名称（或路径）
```

#### 示例：
1. **为文件创建软连接**：  
   假设当前目录有一个文件 `test.txt`，创建一个名为 `link_to_test` 的软连接指向它：
   ```bash
   ln -s test.txt link_to_test
   ```
   此时，`link_to_test` 就是 `test.txt` 的软连接，通过 `cat link_to_test` 可以直接访问 `test.txt` 的内容。

2. **为目录创建软连接**：  
   假设存在目录 `docs`，创建一个名为 `doc_link` 的软连接指向它：
   ```bash
   ln -s ./docs doc_link
   ```
   进入 `doc_link` 等同于进入 `docs` 目录：`cd doc_link`。

3. **使用绝对路径创建软连接**（推荐，避免路径依赖）：  
   如果目标文件的路径是 `/home/user/data/file.dat`，在当前目录创建软连接：
   ```bash
   ln -s /home/user/data/file.dat data_link
   ```


### 注意事项：
- 创建软连接时，**目标文件的路径如果是相对路径，是相对于软连接所在的目录，而非执行命令的目录**。因此，建议优先使用绝对路径，避免移动软连接后失效。
- 查看软连接的指向：可以用 `ls -l` 命令，例如 `ls -l link_to_test`，输出中会显示 `link_to_test -> test.txt`。
- 删除软连接：直接用 `rm 软连接名称` 即可（注意不要加 `/`，否则可能误删目标目录内容），例如 `rm link_to_test`。

通过软连接，可以方便地在不同位置“引用”同一个文件或目录，常用于简化路径访问、版本管理等场景。

在 Linux/Unix 系统中，**硬链接（Hard Link）** 是文件系统中的一个特殊条目，它与目标文件共享同一个**inode**（文件系统中用于存储文件元数据的数据结构，包含文件大小、权限、数据块位置等信息）。简单来说，硬链接相当于目标文件的“别名”，它们指向同一个实际数据。


### 硬链接的特点：
1. **共享 inode**：硬链接与目标文件拥有相同的 inode 编号，因此本质上是同一个文件的不同名称。
2. **不能跨文件系统**：由于 inode 仅在单个文件系统内有效，硬链接无法指向其他分区或文件系统的文件。
3. **不能指向目录**：出于防止目录循环（如递归指向父目录）和维护文件系统一致性的目的，硬链接不支持指向目录。
4. **目标文件删除不影响硬链接**：即使删除了原文件，硬链接仍能正常访问文件数据（因为 inode 会在所有硬链接都被删除后才释放数据块）。
5. **权限与原文件一致**：硬链接的权限、所有者等属性与目标文件完全相同（因为共享 inode）。
...
#### 示例：
1. **为文件创建硬链接**：  
   假设当前目录有文件 `test.txt`，创建一个名为 `hard_link_to_test` 的硬链接：
   ```bash
   ln test.txt hard_link_to_test
   ```
   此时，`test.txt` 和 `hard_link_to_test` 指向同一个 inode，修改其中一个文件的内容，另一个会同步变化。

2. **查看硬链接的 inode 信息**：  
   用 `ls -li` 命令可以看到两者的 inode 编号相同：
   ```bash
   ls -li test.txt hard_link_to_test
   # 输出示例：12345 -rw-r--r-- 2 user user 0 Oct 14 10:00 hard_link_to_test
   #           12345 -rw-r--r-- 2 user user 0 Oct 14 10:00 test.txt
   # （第一个数字是 inode 编号，两者相同；数字“2”表示该 inode 有 2 个硬链接）
   ```

3. **删除原文件后硬链接仍有效**：  
   删除 `test.txt` 后，`hard_link_to_test` 依然可以访问文件内容：
   ```bash
   rm test.txt
   cat hard_link_to_test  # 仍能正常读取内容
   ```


### 注意事项：
- 硬链接**不能指向目录**，执行 `ln dir/ link_dir` 会报错。
- 硬链接**不能跨文件系统**，例如无法从 `/` 分区指向 `/mnt/usb`（U盘）中的文件。
- 删除硬链接使用 `rm 硬链接名称`，不会影响原文件（除非删除了最后一个硬链接）。
- 硬链接适合需要在不同位置访问同一文件，且不希望因原文件删除而失效的场景（如重要配置文件的多位置引用）。


### 软连接 vs 硬链接的核心区别：
| 特性               | 软连接（符号链接）       | 硬链接                   |
|--------------------|--------------------------|--------------------------|
| 本质               | 独立文件，存储目标路径   | 与目标文件共享 inode     |
| 跨文件系统         | 支持                     | 不支持                   |
| 指向目录           | 支持                     | 不支持                   |
| 目标删除后状态     | 失效（断链）             | 仍有效（只要有硬链接存在）|
| 权限显示           | 固定为 `lrwxrwxrwx`      | 与目标文件完全一致       |

根据实际需求选择链接类型：需要灵活跨路径或指向目录时用软连接；需要文件数据“永久”关联且不跨分区时用硬链接。
