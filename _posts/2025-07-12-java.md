...
>参考：[10分钟速成Java](https://www.bilibili.com/video/BV1Ee411H7mT/)、[JavaGuidePro](https://javaguidepro.com/java-basic/java-basic/)、[JAVA基础——接口](https://blog.csdn.net/qq_19782019/article/details/80259836)  
...
//字符串非基本类型
String string1 = "abc";//String对象不可变
...
```
增强型 for 循环写法 *(用于遍历数组或实现了 Iterable 接口的集合类)*
```java
for(char item : dd) {
...
public class Main {
    public static void main(String[] args) {
        ...
        // 实现回调接口（匿名内部类）
        task.execute(/*
            //这里传递的是一个匿名内部类对象，它实现了Callback接口。
            new Callback() { 
                @Override
